        ...
        (ARGON_SECRET,
                    argon2::Algorithm::default(),
                          argon2::Version::default(),
                             argon2::Params::default()).unwrap();

}

/// user 表对应的数据结构，处理 login/registerpub
struct UserDb {
    pool: SqlitePool,
}
...
impl UserDb {
    ...
    pub async fn register(&self, email: &str, password: &str) -> Result<i64> {
        let hashed_password = generate_password_hash(password.as_bytes())?;
    ...
    pub async fn login(&self, email: &str, password: &str) -> Result<String> {
        ...
        if let Err(_) = verify_password(password.as_bytes(), &user.hashed_password) {
...
fn verify_password(password: &[u8], password_hash: &str) -> Result<()> {
    let parsed_hash =
...
async fn main() -> Result<()> {
    ...
    let email = "wiccy@TheCowBoy.com";
    let password = "whoknows";
    ...
    println!("Login in succeeded: {token}");

    // 登录失败
    let result = user_db.login(email, "wrong").await;
